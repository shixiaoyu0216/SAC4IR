...
class LazyMemory(dict):
    ...
    def _sample(self, indices, batch_size):
        ...
        states = np.empty((batch_size, self.state_shape), dtype=np.int32)
        next_states = np.empty((batch_size, self.state_shape), dtype=np.int32)
        ...
        actions = torch.from_numpy(self['action'][indices]).to(self.device)
        rewards = torch.from_numpy(self['reward'][indices]).to(self.device)
        dones = torch.from_numpy(self['done'][indices]).to(self.device)
